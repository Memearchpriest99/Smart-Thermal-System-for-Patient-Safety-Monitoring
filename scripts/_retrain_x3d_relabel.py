"""Retrain Thermo-X3D on the FULL corrected dataset (post re-annotation).

All 17 scenes, corrected labels (224 positives). Same protocol as
eval_waveshare_contact_dl.py (timeline split 60/15/25, class-weighted CE,
val threshold sweep). Checkpoint -> Waveshare_26984_relabel.thalg.
Then a pooled full-replay benchmark vs config-D reference.

Outputs reports/x3d_relabel_results.json.
"""
...
dl.X3D_CKPT = _ROOT / "checkpoints" / "thermo_x3d_detector" / "Waveshare_26984_relabel.thalg"
...
print("building sequences (ALL scenes, corrected labels) ...", flush=True)
...
print(f"  {len(seqs)} scenes, {sum(len(v) for v in seqs.values())} frames "
      f"({npos} pos) | train pos {ptr}", flush=True)

res = dl.run_thermo_x3d(seqs)

# ---- pooled full-replay with reloaded ckpt ---------------------------------
...
for scene, recs in seqs.items():
    ...
    scenes[scene] = ([r["label"] for r in recs], [1 if c > th else 0 for c in confs])

def pooled(tol):
    tp = fn = fp = tn = 0
    for labs, yp in scenes.values():
...
out = {"threshold": th, "honest": res, "full_replay": {"tol0": m0, "tol2": m2},
       "per_scene_f1": per_scene}
out_path = _ROOT / "reports" / "x3d_relabel_results.json"
